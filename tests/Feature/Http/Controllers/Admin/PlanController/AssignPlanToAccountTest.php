...
use function Tests\Feature\Http\Controllers\Admin\PlanController\Helpers\populateUser;
...
it('can assign plan to branch', function ($data) {
    ...
    $response = $this->put(route('admins.branch-plan.update', $branch->id), [
        'plans' => [$plan->id],
    ]);
    ...
    expect($branch->plans()->count())->toBe(1)
        ...
        ->and($subscription->amount)->toBe($data['amount'])
        ->and($subscription->attributes)->toBe($data['attributes'])
        ->and($subscription->attributes['name'])->toBe($data['attributes']['name'])
        ->and($subscription->attributes['type'])->toBe($data['attributes']['type'])
        ->and($subscription->attributes['attribute'])->toBe($data['attributes']['attribute'])
        ->and($subscription->attributes['attribute'][0]['name'])->toBe($data['attributes']['attribute'][0]['name'])
        ->and($subscription->attributes['attribute'][0]['value'])->toBe($data['attributes']['attribute'][0]['value'])
        ->and($subscription->status)->toBe($data['status'])
        ->and($subscription->duration)->toBe($data['duration'])
...
it('can subscription to plan', function ($data) {

    [$branch , $user] = populateUser();

    $this->actingAs($user)
        ->withSession(['current_branch', $branch]);

    $plan = \App\Models\Plan::factory($data)->create();

    $response = $this->get(route('account.subscriptions.show', $plan->id));
