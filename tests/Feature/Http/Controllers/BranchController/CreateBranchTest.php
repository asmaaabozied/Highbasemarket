...
it('can create a branch', function ($data) {

    fake_file_envirenment();

    [$account , $current_branch , $user] = prapperData(0);
    ...
    $data['config']['delivery_locations'] = [
        [
    ...
    $response = $this->post(route('account.branches.store'), $data);

    $response->assertStatus(302)
        ->assertSessionDoesntHaveErrors();

    $branch = \App\Models\Branch::query()->latest()->first();
    ...
    'Test With images' => [
        [
            ...
            'cr'           => 'update Name',
            'tax_number'   => '12344',
            'image'        => 'http://placehold.it/640x480.png/00ff00?text=Created+Branch',
            'cr_image'     => 'http://placehold.it/640x480.png/00ff00?text=Created+Branch',
            'cover_image'  => 'http://placehold.it/640x480.png/00ff00?text=Created+Branch',
            'phone'        => ['code' => '232', 'number' => '2322222'],
            'address'      => [
                'country' => '1234',
                'state'   => '1234',
...
it('can validate rules', function ($field) {
    ...
    $response = $this->post(route('account.branches.store'), [...$data, 'account_id' => $account->id]);
