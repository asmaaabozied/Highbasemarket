...
describe('can reject Order', function () {
    it('has administrator job_tile', function () {
        ...
        $this->actingAs($user);
        $response = $this->put(route('account.orders.reject', ['order' => $order]),
            [
                'lines'             => $order->lines->pluck('id')->toArray(),
                'rejection_message' => 'not interested',
            ]
        );
        ...
        expect($order->lines->pluck('status')->first())->toBe('rejected');
    });

    it('has `reject order` permission', function () {
        [$account, $branch, $user] = createAccount('employee');
        populateProducts();
        addPermissions($user, ['reject order'], 'account', 'order');
...
describe('cannot reject Order', function () {
    ...
    it('dose not have `reject order` permission', function () {
        [$account, $branch, $user] = createAccount('employee');
        populateProducts();
