...
describe('can ship Order', function () {
    it('has administrator job_tile', function () {
        ...
        $this->actingAs($user);
        $response = $this->put(route('account.orders.ship', ['order' => $order]),
            [
        ...
        expect($order->lines->pluck('status')->first())->toBe('shipped');
    });

    it('has `ship order` permission', function () {
        [$account, $branch, $user] = createAccount('employee');
        populateProducts();
        addPermissions($user, ['ship order'], 'account', 'order');
...
describe('cannot ship Order', function () {
    ...
    it('dose not have `ship order` permission', function () {
        [$account, $branch, $user] = createAccount('employee');
        populateProducts();
    ...
    it('has orderLine with non approved status', function ($status) {
        [$account, $branch, $user] = createAccount();
        populateProducts();
